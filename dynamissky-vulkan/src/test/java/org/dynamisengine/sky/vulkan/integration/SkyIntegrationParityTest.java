...
class SkyIntegrationParityTest {
    ...
    private static final class FakeGpuMemoryOps implements GpuMemoryOps {
        ...
        public GpuImage2DAlloc createImage2D(int width, int height, int format) {
            long image = handle++;
            long memory = handle++;
            return new GpuImage2DAlloc(new org.dynamisengine.sky.api.gpu.GpuImage2D(image, width, height), memory, format);
        }
        ...
        public GpuImage3DAlloc createImage3D(int width, int height, int depth, int format) {
            long image = handle++;
            long memory = handle++;
            return new GpuImage3DAlloc(new org.dynamisengine.sky.api.gpu.GpuImage3D(image, width, height, depth), memory, format);
        }
